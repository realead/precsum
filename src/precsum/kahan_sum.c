...
#define ROW_COUNT 32
void kahan_blocksum_FLOAT(const value_t *a, index_t n, index_t stride_along, index_t m, index_t stride_crosswise, value_t *output, index_t stride_output)
{
    //we trust m to be<=ROW_COUNT
    if (n < 8) {
        index_t j;
        for( j = 0; j < m; j++){
               output[j*stride_output]=naive_kahan_1dsum_FLOAT(a+j*stride_crosswise,n,stride_along);
        }
    }
    else{
        index_t i,j;
        ...
        for (i = 8; i < n - (n % 8); i += 8) {
            ...
            for( j = 0; j < m; j++){
                y[j][0] = a[(i + 0) * stride_along+j*stride_crosswise]-c[j][0];
                y[j][1] = a[(i + 1) * stride_along+j*stride_crosswise]-c[j][1];
                y[j][2] = a[(i + 2) * stride_along+j*stride_crosswise]-c[j][2];
                y[j][3] = a[(i + 3) * stride_along+j*stride_crosswise]-c[j][3];
                y[j][4] = a[(i + 4) * stride_along+j*stride_crosswise]-c[j][4];
                y[j][5] = a[(i + 5) * stride_along+j*stride_crosswise]-c[j][5];
                y[j][6] = a[(i + 6) * stride_along+j*stride_crosswise]-c[j][6];
                y[j][7] = a[(i + 7) * stride_along+j*stride_crosswise]-c[j][7];
         ...
         for( j = 0; j < m; j++){
            sum[j][8] = naive_kahan_1dsum_FLOAT(a+i*stride_along+j*stride_crosswise,n-i,stride_along);
...
void kahan_2dsum_FLOAT(const value_t *a, index_t n, index_t stride_along, index_t m, index_t stride_crosswise, value_t *output, index_t stride_output){
    while(m>ROW_COUNT){
        kahan_blocksum_FLOAT(a, n, stride_along, ROW_COUNT, stride_crosswise, output, stride_output);
        a+=stride_crosswise*ROW_COUNT;
        output+=stride_output*ROW_COUNT; 
        m-=ROW_COUNT; 
    }
    kahan_blocksum_FLOAT(a, n, stride_along, m, stride_crosswise, output, stride_output);
}
