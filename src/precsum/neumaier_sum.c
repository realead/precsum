...
void neumaier_blocksum_FLOAT(const value_t *a, index_t n, index_t stride_along, index_t m, index_t stride_crosswise, value_t *output, index_t stride_output)
{
    ...
    if (n < 8) {
        ...
        for (i = 8; i < n - (n % 8); i += 8) {
            ...
            for( j = 0; j < m; j++){
                t[j][0] = sum[j][0]+a[(i + 0) * stride_along+j*stride_crosswise];
                t[j][1] = sum[j][1]+a[(i + 1) * stride_along+j*stride_crosswise];
                t[j][2] = sum[j][2]+a[(i + 2) * stride_along+j*stride_crosswise];
                t[j][3] = sum[j][3]+a[(i + 3) * stride_along+j*stride_crosswise];
                t[j][4] = sum[j][4]+a[(i + 4) * stride_along+j*stride_crosswise];
                t[j][5] = sum[j][5]+a[(i + 5) * stride_along+j*stride_crosswise];
                t[j][6] = sum[j][6]+a[(i + 6) * stride_along+j*stride_crosswise];
                t[j][7] = sum[j][7]+a[(i + 7) * stride_along+j*stride_crosswise];
                 
                c[j][0]+=fabs(sum[j][0])>=fabs(a[(i+0)*stride_along+j*stride_crosswise]) ? (sum[j][0]-t[j][0])+a[(i+0)*stride_along+j*stride_crosswise] : (a[(i+0)*stride_along+j*stride_crosswise]-t[j][0])+sum[j][0];
                c[j][1]+=fabs(sum[j][1])>=fabs(a[(i+1)*stride_along+j*stride_crosswise]) ? (sum[j][1]-t[j][1])+a[(i+1)*stride_along+j*stride_crosswise] : (a[(i+1)*stride_along+j*stride_crosswise]-t[j][1])+sum[j][1];
                c[j][2]+=fabs(sum[j][2])>=fabs(a[(i+2)*stride_along+j*stride_crosswise]) ? (sum[j][2]-t[j][2])+a[(i+2)*stride_along+j*stride_crosswise] : (a[(i+2)*stride_along+j*stride_crosswise]-t[j][2])+sum[j][2];
                c[j][3]+=fabs(sum[j][3])>=fabs(a[(i+3)*stride_along+j*stride_crosswise]) ? (sum[j][3]-t[j][3])+a[(i+3)*stride_along+j*stride_crosswise] : (a[(i+3)*stride_along+j*stride_crosswise]-t[j][3])+sum[j][3];
                c[j][4]+=fabs(sum[j][4])>=fabs(a[(i+4)*stride_along+j*stride_crosswise]) ? (sum[j][4]-t[j][4])+a[(i+4)*stride_along+j*stride_crosswise] : (a[(i+4)*stride_along+j*stride_crosswise]-t[j][4])+sum[j][4];
                c[j][5]+=fabs(sum[j][5])>=fabs(a[(i+5)*stride_along+j*stride_crosswise]) ? (sum[j][5]-t[j][5])+a[(i+5)*stride_along+j*stride_crosswise] : (a[(i+5)*stride_along+j*stride_crosswise]-t[j][5])+sum[j][5];
                c[j][6]+=fabs(sum[j][6])>=fabs(a[(i+6)*stride_along+j*stride_crosswise]) ? (sum[j][6]-t[j][6])+a[(i+6)*stride_along+j*stride_crosswise] : (a[(i+6)*stride_along+j*stride_crosswise]-t[j][6])+sum[j][6];
                c[j][7]+=fabs(sum[j][7])>=fabs(a[(i+7)*stride_along+j*stride_crosswise]) ? (sum[j][7]-t[j][7])+a[(i+7)*stride_along+j*stride_crosswise] : (a[(i+7)*stride_along+j*stride_crosswise]-t[j][7])+sum[j][7];
         ...
         for( j = 0; j < m; j++){
             ...
             sum[j][8] = naive_neumaier_1dsum_FLOAT(a+i*stride_along+j*stride_crosswise,n-i,stride_along);
